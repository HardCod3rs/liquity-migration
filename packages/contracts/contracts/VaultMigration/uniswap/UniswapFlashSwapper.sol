...
contract UniswapFlashSwapper {
    ...
    constructor(
        ...
        uniswapV2Factory = IUniswapV2Factory(UniswapFactory);
        WETH = _WETH;
        DAI = _DAI;
    }
    ...
    function simpleFlashLoanExecute(
        ...
        _migrateVault(tokenBorrowed, _amount, tokenToRepay, amountToRepay, _userData);
    ...
    function simpleFlashSwapExecute(
        ...
        address pairAddress = permissionedPairAddress; // gas efficiency
        uint256 pairBalanceTokenBorrow = IERC20(_tokenBorrow).balanceOf(pairAddress);
        uint256 pairBalanceTokenPay = IERC20(_tokenPay).balanceOf(pairAddress);
        uint256 amountToRepay = ((1000 * pairBalanceTokenPay * _amount) / (997 * pairBalanceTokenBorrow)) + 1;
    ...
    function traingularFlashSwap(
        ...
        uint256 pairBalanceTokenBorrowBefore = IERC20(_tokenBorrow).balanceOf(borrowPairAddress);
        ...
        uint256 amountOfWeth = ((1000 * pairBalanceWeth * _amount) / (997 * pairBalanceTokenBorrowAfter)) + 1;
    ...
    function traingularFlashSwapExecute(
        ...
        (address _borrowPairAddress, uint256 _amountOfWeth) = abi.decode(_triangleData, (address, uint256));
        ...
        uint256 pairBalanceTokenPay = IERC20(_tokenPay).balanceOf(payPairAddress);
        uint256 amountToRepay = ((1000 * pairBalanceTokenPay * _amountOfWeth) / (997 * pairBalanceWETH)) + 1;

        // Step 4: Do whatever the user wants (arb, liqudiation, etc)
        _migrateVault(_tokenBorrow, _amount, _tokenPay, amountToRepay, _userData);
